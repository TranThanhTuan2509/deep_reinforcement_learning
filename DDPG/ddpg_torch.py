...
class RelayBuffer(object):
	self.__init__(self, max_size, input_shape, n_actions):
...
class CriticNetwork(nn.Module):
	def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name,
		...
		# Init fc1 weights with uniform distribution
	...
	def save_checkpoint(self):
		...
		T.save(self.state_dict(), slef.checkpoint_file)
...
class ActorNetwork(nn.Module):
	def __init__(self, alpha, input_dims, fc1_dims, fc2_dims, n_actions, name,
		...
		self.n_actions = n_actions
		self.checkpoint_file = os.path.join(chkpt_dir, name+'_ddpg')
		self.fc2 = nn.Linear(*self.input_dims, self.fc1_dims)
		f1 = 1 / np.sqrt(self.fc1.weight.data.size()[0])
		T.nn.init.uniform_(self.fc1.weight.data, -f1, f1)
		T.nn.init(uniform_(self.fc1.bias.data, -f1, f1))
		self.bn1 = nn.LayerNorm(self.fc1_dims)
...
class Agent(object):
	def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99, n_actions=2,
		...
		self.noise = QUActionNoise(mu=np.zeros(n_actions))
