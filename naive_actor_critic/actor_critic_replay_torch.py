...
class RelayBuffer():
	def __init__(self, max_size, input_shape):
		...
		self.mem_cntr = 0
		self.state_memory = np.zeors((self.mem_size, *input_shape), dtype = np.float32)
		self.new_state_memory = np.zeors((self.mem_size, *input_shape), dtype = np.float32)
...
class Agent():
	def __init__(self, lr, input_dims, n_actions, gamma=0.99, l1_size = 256, 
		l2_size=256, batch_size= 32, mem_size=100000):
		self.gamma = gamma
		self.batch_size = batch_size
		self.memory = RelayBuffer(mem_size, input_dims)
		self.actor_critic = ActorCriticNetwork(le, input_dims, l1_size, l2_size
			n_actions=n_actions)
	...
	def choose_action(self, observation):
		...
		action_probs = T.distribution.Categorical(probabilities)
	...
	def learn(self):
		...
		done = T.tensor(done).to(self.actor_critic.device)
		...
		cirtic_loss = F.mse_loss(delta, critic_value)

		(actor_loss + cirtic_loss).backward()
