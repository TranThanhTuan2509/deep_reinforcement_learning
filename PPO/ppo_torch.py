...
class PPOMemory:
    ...
    def store_memory(self, state, action, probs, vals, reward, done):
        ...
        self.actions.append(action)  # Missing 'actions' append in original
...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha, fc1_dims=256, fc2_dims=256, chkpt_dir='tmp/ppo'):
        ...
        self.actor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(fc2_dims, n_actions)  # Output logits, no Softmax
        )
    ...
    def forward(self, state):
        ...
        dist = Categorical(logits=dist)  # Use logits explicitly
        return dist
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=256, fc2_dims=256, chkpt_dir='tmp/ppo'):
        ...
        self.critic = nn.Sequential(
            ...
            nn.Linear(fc1_dims, fc2_dims),
            nn.ReLU(),
            nn.Linear(fc2_dims, 1)  # Single value output
        )
        self.optimizer = optim.Adam(self.parameters(), lr=alpha)
        self.device = T.device('cuda:0' if T.cuda.is_available() else 'cpu')
        self.to(self.device)
