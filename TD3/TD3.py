...
class RelayBuffer():
	...
	def sample_buffer(self, batch_size):
		...
		return states, actions, rewards, state_, dones
...
class CriticNetwork(nn.Module):
	def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir='tmp/td3'):
		...
		self.checkpoint_file = os.path.joint(self.checkpoint_dir, name+'_td3')

		self.fc1 = nn.Linear(self.input_dims[0] + n_actions, self.fc1_dims)
		self.fc2 = nn.Linear(self.fc1_dims, self.fc2_dims)
		self.q1 = nn.Linear(self.fc2_fims, 1)
	...
	def forward(self, state, action):
		q1_action_value = self.fc1(T.cat[state, actino], dim = 1)
		q1_action_value = F.relu(q1_action_value)
		q1_action_value = self.fc2(q1_action_value)

		q1_action_value = R.relu(q1_action_value)
...
class ActorNetwork(nn.Modlule):
	def __init__(self, alpha, input_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir='tmp/td3'):
		...
		self.name = name
		self.checkpoint_dir = chkpt_dir
		self.checkpoint_file = os.path.join(self.checpoint_dir, name+'_td3')
...
class Agent():
	def __init__(self, alpha, beta, input_Dims, tau, env, 
		...
		self.gamma=gamma
		self.tau = tau
		self.max_action = env.actions_space.high
		self.min_actions = env.actions_space.low
		self.memory= RelayBuffer(max_size, input_dims, n_actions)
	...
	def choose_action(self, observation):
		if self.time_step < self.warmup:
			mu = T.tensor(np.random.normal(scale = slef.noise, 
											size=(self.n_actions,)))

		else:
			state = T.tensor(observation, stype=T.float).to(self.actor.device)
	...
	def learn(self):
		...
		state_ = T.tensor(new_state_memory, dtype = T.float).to(self.actor.device)
		...
		target_actions = T.clamp(target_actions, self.min_actions[0],
										self.max_action[0])
		# Guarantees the action still in determined range
		# SAC = tanh(target_action)*max_action
		...
		# target update of Clipped Double Q-learning algorithm:
